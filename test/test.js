...
describe("Testing the add Request",function(err)
{
  it("should handle the request and add a new object and Redirect",function(done)
  {
      url
        .post("/curdOpt/add")
        .send({"Title":"Thani Oruvan","Year":"2015","Actors":"Jeyam Ravi, Nayanthara, Arvind Swamy, Mugdha Godse","Director":"M. Raja","Released":"28 Aug 2015","Plot":"Revolving around a police officer who is passionate to destroy the most corrupt person in the country, Siddharth Abhimanyu","imdbRating":"8.9","Awards":"N/A","imageurl":"thanioruvan.jpg"})
        .expect(302)
        .end(function(err,res)
        {
          if(err)
    ...
    it("Checking whether the object is added",function(done)
    {
        url
          ...
          .end(function(err,res)
          {
            ...
            var myObj=JSON.parse(res.text);
            myObj[myObj.length-1].Title.should.be.equal("Thani Oruvan");
            done();
          });
  });
}); */
...
describe("Testing the Delete Request",function(err)
{
  ...
  {
      url
        .delete("/curdOpt/deletePage/Thani Oruvan")
        //.send({"Title":"Thani Oruvan","updRating":"9.0","updAwards":"1 National Award","imageurl":"thanioruvan.jpg"})
        .expect(302)
        .end(function(err,res)
        {
